...
const getQueryByIdAndUpdate = async(queryId, options) => {
    return await Query.findOneAndUpdate({ _id: queryId },
        options, { new: true }
    );
}
...
const addSolvedQuestionToUser = async(username, quesId) => {
    return await User.findOneAndUpdate({ username }, { $addToSet: { solvedQuestions: quesId } }, { new: true });
}
const incrTotalSubmInUser = async(userId) => {
    return await User.findByIdAndUpdate(
        userId, { $inc: { totalSubmissions: 1 } }, { new: true }
    );
}
